...
# 题目分析
序列化树，序列化图本质上都是序列化，用BFS遍历每一个节点然后想一个数据结构存下来（序列化）。具体可以是list，dict看哪种方便，然后再想办法还原。克隆树，克隆图，方法相似，不用序列化，但是用相似的方法，找一种数据结构存下来，再深度拷贝（或者说最愚蠢的克隆方法就是先序列化再还原，事实上网络通信中就是这样干的）
